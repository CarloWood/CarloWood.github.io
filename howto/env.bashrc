...
function __cdeh_store_environment ()
{
  declare -p | awk '
      ...
      /^declare -/ {
        ...
        gsub("=.*","",name);
        gsub("-+", "-g", $2);
        show=!index($2,"r") && !match(name, "^((__cdeh_|CDEH|BASH)|(name|HISTFILE|HISTFILESIZE|HISTSIZE|PIPESTATUS|PWD|_)$)")
      }
      {
        if (show)
          print
...
function __cdeh_clear_environment ()
{
  ...
  for name in $(declare -p | \
                ...
                /bin/sed -e 's/=.*//;s/.* //' | \
                /bin/egrep -v '^((__cdeh_|CDEH_|BASH_)|(BASH|BASHPID|BASHOPTS|COMP_WORDBREAKS|DIRSTACK|FUNCNAME|GROUPS|LINENO|RANDOM|SECONDS|HOME|PATH|PS1|PROMPT_COMMAND|PWD|name|HISTFILE|HISTFILESIZE|HISTSIZE|HISTCMD|HISTCONTROL|PIPESTATUS|HOSTFILE|MAILCHECK|_)$)'); do
    unset $name
  done
